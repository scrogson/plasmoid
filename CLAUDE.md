...
- **Dial by address, not by bare node id.** A bare `EndpointId` asks an address-lookup service to *find* the peer: pkarr/DNS publishes an empty address set for the first few seconds after binding, and mDNS needs multicast that macOS gates. Every dial goes through `PeerLinks::dial_target`, and `Announce` carries `EndpointAddr`s so a node learned transitively is also reachable. Cross-node tests call `Runtime::knows` or `join_at` rather than depending on discovery — that is why they now run offline, in ~2s instead of timing out.
