...
**Supervision** — **works**. A supervisor is an ordinary particle: the policy is a pure decision core (`crates/plasmoid-sdk/src/supervisor.rs`, 17 host-side tests) and `run_supervisor!` drives it against the host, starting children with `spawn-link`, restarting them per strategy and restart type, honouring shutdown, and giving up when restart intensity is exceeded. `components/supervised` is a working example and is exercised end to end in CI. **Not built**: the manifest and CLI boot path from [#41](https://github.com/scrogson/plasmoid/issues/41), so a tree is started by spawning its root rather than declared; and dynamic children ([#44](https://github.com/scrogson/plasmoid/issues/44)). The primitives a supervisor would be built from — `link`, `monitor`, `trap-exit`, `exit-signal`, and exit-signal propagation — are all in place and working, including the untrappable kill behind OTP's `brutal_kill`. Supervisors are intended to be ordinary particles rather than a runtime feature, as in OTP.
